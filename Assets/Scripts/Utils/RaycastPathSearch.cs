...
namespace MyGame
{
    public class RaycastPathSearch : MonoBehaviour
    {
        ...
        static public List<Vector3> GetPath(Vector3 start, Vector3 goal, int layerMask)
        {
            ...
            while (true)
            {
                ...
                if (!BounceObject.BounceObjectList.ContainsKey(hitA.collider.gameObject))
                {
                ...
                while (true)
                {
                    ...
                    {
                        if (!BounceObject.BounceObjectList.ContainsKey(hitB.collider.gameObject))
                        {
                            return new List<Vector3>() { start };
                        }

...
public class Tool_MazeLikeMapGenerator
{
    static public bool[,] Generate(int row, int col)
    {
        ...
        var map = Tool_MazeGenerator.Generate(row, col, reservedPoints: reservedPoints);
    ...
    static List<MapSpace> GetRandomSpaceList(int row, int col)
    {
        ...
        static bool Check(int r, int c, bool[,] reserved)
        {
            ...
            for (int n = 0; n < Utility.PanelPoints.GetLength(0); n++)
            {
                int rr = r + Utility.PanelPoints[n, 0];
                int cc = c + Utility.PanelPoints[n, 1];
        ...
        static bool DecideRoomOrNot()
        {
            ...
            if (val < Floats.Get(Floats.Item.sv_room_space_ratio))
            {
                return true;
            }

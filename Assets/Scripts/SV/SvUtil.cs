...
namespace MyGame
{
    public class SvUtil : MonoBehaviour
    {
        ...
        static public int BounceLayer { get; } = 1 << 8;
        static public int DropItemLayer { get; } = 1 << 9;
        static public int PlayerLayer { get; } = 1 << 10;

        static public float CalcDeltaTheta(float degTheta1, float degTheta2)
        {
            degTheta1 = CorrectRotationAngle(degTheta1);
            degTheta2 = CorrectRotationAngle(degTheta2);

            float direction = 1.0f;

            if (degTheta1 > degTheta2)
            {
                direction = -1.0f;
            ...
            float deltaTheta1 = degTheta2 - degTheta1;
            float deltaTheta2 = (degTheta1 + 360.0f) - degTheta2;

            if (deltaTheta1 < deltaTheta2)
            {
                return direction * deltaTheta1;
            }

            else
            {
                return direction * deltaTheta2 * (-1.0f);
            }

            // - inner function
            static float CorrectRotationAngle(float rot)
            {
                if (rot > 0)
                {
                    return rot % 360.0f;
        ...
        {
            bool[,] noObject = ShareSystem.Passable;
            ...
            for (int c = 0; c < mazeCol; c++)
            {
                for (int r = 0; r < mazeRow; r++)
                {
                    ...
                    if (exept != null)
                    {
                        if (Contain(exept, new int[2] { r, c })) { continue; }
                    }
            ...
            Dictionary<T, float> spawnRateList, T defaultValue, int maxSpawns)
        {
            ...
            // function
            static List<T> GetRandomObjectList<T>(Dictionary<T, float> spawnRateList, List<int[]> candidatePointList, int maxDrops, T defaultValue)
            {
                var _spawnRateList = new Dictionary<T, float>(spawnRateList);

                var normalizedDropRateList = GetNormalizedRatioList(_spawnRateList);
                ...
                while (true)
                {
                    ...
                    if (n_dropList[item] >= maxDropList[item])
                    {
                        ...
                        normalizedDropRateList = GetNormalizedRatioList(normalizedDropRateList);
                    }
                }
                ...
                // function
                static Dictionary<T, float> GetNormalizedRatioList<T>(Dictionary<T, float> dropRateList)
                {
                    var sum = 0.0f;

...
public class Tool_MazeGenerator
{
    class Node
    {
        public enum Status
    ...
    static public bool[,] Generate(int row, int col, List<int[]> reservedPoints = null)
    {
        ...
        for (int roopA = 0; roopA < int.MaxValue; roopA++)
        {
            ...
            for (int roopB = 0; roopB < int.MaxValue; roopB++)
            {
                ...
                var nextIndex = new int[2] { baseIndex[0] + Utility.CrossPoints[dir, 0], baseIndex[1] + Utility.CrossPoints[dir, 1] };
    ...
    static void CloseNodes(int[] baseIndex)
    {
        for (int roopC = 0; roopC < int.MaxValue; roopC++)
        {
            ...
            for (int n = 0; n < 4; n++)
            {
                if (dz != Utility.CrossPoints[n, 0] || dx != Utility.CrossPoints[n, 1]) { continue; }
        ...
        for (int x = 0; x < nodeCol; x++)
        {
            for (int z = 0; z < nodeRow; z++)
            {
                ...
                for (int n = 0; n < 4; n++)
                {
                    ...
                    int zz = 2 * z + Utility.CrossPoints[n, 0];
                    int xx = 2 * x + Utility.CrossPoints[n, 1];
    ...
    static bool[,] CorrectMaze(bool[,] maze)
    {
        ...
        for (int c = 1; c < (col - 1); c++)
        {
            for (int r = 1; r < (row - 1); r++)
            {
                ...
                for (int n = 0; n < 4; n++)
                {
                    var cross1 = new int[2] { Utility.CrossPoints[(n + 0) % 4, 0], Utility.CrossPoints[(n + 0) % 4, 1] };
                    var cross2 = new int[2] { Utility.CrossPoints[(n + 1) % 4, 0], Utility.CrossPoints[(n + 1) % 4, 1] };

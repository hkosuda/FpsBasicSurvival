...
namespace MyGame
{
    ...
    public class SV_Shop : HostComponent
    {
        ...
        {
            NextMaxHP = GetNextValue(ShopItem.hp_upgrade, SV_Status.CurrentMaxHP, Params.shop_hp_upgrade_amount);
            NextMaxArmor = GetNextValue(ShopItem.armor_upgrade, SV_Status.CurrentMaxArmor, Params.shop_armor_upgrade_amount);
        ...
        {
            NextHP = GetNextValue_Clip(ShopItem.hp_healing, SV_Status.StatusList[Status.hp], Params.shop_hp_healing_amount, NextMaxHP);
            NextArmor = GetNextValue_Clip(ShopItem.armor_repairing, SV_Status.StatusList[Status.armor], Params.shop_armor_repairing_amount, NextMaxArmor);
            NextDamageRate = GetNextValue(ShopItem.damage_rate_booster, SV_Status.CurrentDamageRate, Params.shop_damage_rate_booster_amount);
            NextMoneyRate = GetNextValue(ShopItem.money_rate_booster, SV_Status.CurrentMoneyRate, Params.shop_money_rate_booster_amount);
        }
        ...
        {
            SubFromCart(ShopItem.hp_healing, NextMaxHP, SV_Status.StatusList[Status.hp], Params.shop_hp_healing_amount);
            SubFromCart(ShopItem.armor_repairing, NextMaxArmor, SV_Status.StatusList[Status.armor], Params.shop_armor_repairing_amount);
        ...
        {
            SV_Status.StatusList[Status.hp] = NextHP;
            SV_Status.StatusList[Status.armor] = NextArmor;
            SV_Status.StatusList[Status.money] = Mathf.RoundToInt(MoneyRemain * Params.money_increase_after_round);

            SV_Status.CurrentMaxHP = NextMaxHP;
            SV_Status.CurrentMaxArmor = NextMaxArmor;

            SV_Status.CurrentDamageRate = NextDamageRate;
            SV_Status.CurrentMoneyRate = NextMoneyRate;

...
namespace MyGame
{
    public class HistoryContentManager : MonoBehaviour
    {
        ...
        {
            { SV_History.HistoryValue.movingDistance, "à⁄ìÆãóó£" },
            { SV_History.HistoryValue.destroyed, "åÇîj" },
            { SV_History.HistoryValue.takenDamage, "îÌÉ_ÉÅÅ[ÉW" },
            { SV_History.HistoryValue.givenDamage, "ó^É_ÉÅÅ[ÉW" },
            { SV_History.HistoryValue.gotMoney, "älìæÉ}ÉlÅ[" },
            { SV_History.HistoryValue.shotAmmo, "è¡îÔíeêî" },
        };
        ...
        static void ShowInfo()
        {
            ...
            else
            {
                ...
                {
                    WriteInfo("ÉNÉäÉAÇµÇΩÉâÉEÉìÉhêî", 0);
                    WriteInfo((SV_Round.RoundNumber - 1).ToString(), 1);
                }

                WriteInfo(separator + " ó›åvèÓïÒ " + separator, 0, Clr.lime);
        ...
        static void WriteAllInfo(SV_History.History history, int indent, bool writeValues, bool writeBuy)
        {
            if (writeValues)
            {
                ...
                {
                    WriteInfo("Åy" + title.Value + "Åz", indent);
